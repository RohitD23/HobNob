...
exports.createPost = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (fields.type.toLowerCase() == "friends") {
...
exports.getUserFriendsPosts = (req, res) => {
  ...
  Post.find({
    $or: [{ type: { $eq: "public" } }, { user: { $in: [...friends, _id] } }],
  })
...
exports.updateLikes = (req, res) => {
  ...
  if (req.params.type === "push")
    Post.updateOne(
      { _id: postid },
      { likes: parseInt(req.params.likes), $push: { liked_by: userid } },
      { new: true },
      (err, post) => {
        console.log(err);
...
exports.getAllUserPosts = (req, res) => {
  ...
  Post.find({ user: userId })
    ...
    .exec((err, posts) => {
      if (err || !posts)
        return res.json({ error: "Error while retrieving posts" });
